...
class XUIAPI:
    ...
    async def login(self):
        ...
        try:
            ...
            url = create_endpoint("/login")
            
            logger.info(f"ℹ️  Trying login to {url} with user: {config.XUI_USERNAME}")
            
            async with self.session.post(url, data=auth_data) as resp:
                if resp.status != 200:
    ...
    async def get_inbound(self, inbound_id: int):
        ...
        try:
            url = create_endpoint(f"/panel/api/inbounds/get/{inbound_id}")
    ...
    async def update_inbound(self, inbound_id: int, data: dict):
        """Обновление инбаунда"""
        try:
            url = create_endpoint(f"/panel/api/inbounds/update/{inbound_id}")
    ...
    async def get_user_stats(self, email: str):
        ...
        try:
            url = create_endpoint(f"/panel/api/inbounds/getClientTraffics/{email}")
    ...
    async def get_online_users(self):
        ...
        try:
            url = create_endpoint(f"/panel/api/inbounds/onlines")
...
def create_endpoint(endpoint: str) -> str:
